...
async def autoleave_vc(user_client, call_py, message, duration_str,chat):
    """
    Automatically leave voice chat when only the bot remains in the call for 5 seconds
    """
    session_client = call_py._mtproto
    bot_id = user_client.me.id  # Get bot's own user ID
    ...
    while True:
        ...
        try:
            ...
            members = []
            async for member in session_client.get_call_members(chat.id):
                members.append(member)

            # Check if only bot remains in call
            if len(members) == 1 and members[0].chat.id == session_client.me.id:
                ...
                if len(members) == 1 and members[0].chat.id == session_client.me.id:
                    await call_py.leave_call(chat.id)
                    # Cleanup operations
                    try:
                        queues.get(f"dic_{bot_id}")[chat.id].clear()
                        playing[chat.id].clear()
                    except KeyError:
                        pass
                    
                    await remove_active_chat(user_client, chat.id)
...
async def pautoleave_vc(user_client, call_py, message, duration_str):
    """
    ...
    while True:
        ...
        members = []
        try:
          async for i in session_client.get_call_members(message.chat.id):
            members.append(i)
        except:
           break
...
async def update_progress_button(message, duration_str, call_py, chat):
    try:
        total_seconds = sum(int(x) * 60 ** i for i, x in enumerate(reversed(duration_str.split(":"))))

        while True:
            try:
                updated_msg = await call_py._mtproto.get_messages(message.chat.id,message.id)
            except:
                break
            try:
                # Fetch elapsed seconds
                elapsed_seconds = int(await call_py.time(chat.id))
            except Exception as e:
...
f"[{lightyagami(title)}](https://t.me/{user_client.me.username}?start=vidid_{extract_video_id(youtube_link)})" if not os.path.exists(youtube_link) else lightyagami(title), duration, by.mention()),
            reply_markup=keyboard        )
        asyncio.create_task(autoleave_vc(user_client, call_py, sent_message, duration,chat))
        asyncio.create_task(update_progress_button(sent_message, duration, call_py,chat))
        try:
